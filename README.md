...
match the `addr` you set in a `validator` entry.
...
Declares one blockchain. You need exactly one `chain` entry per chain you
want to sign for.
...
multiple `validator` entries (e.g. primary + backup nodes).
...
| Field | Type | Required | Description |
|---|---|---|---|
| `chain_ids` | list of strings | yes | Chain IDs this key signs for. Each must match a declared `chain[].id`, and each chain may be backed by only one key. |
...
single `kms start` process; you can also run `grpc`-only (no `chain` or
`validator` entries required when there is no privval workload).
