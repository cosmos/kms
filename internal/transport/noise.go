// Package transport provides privval connection transports for kms.
package transport
...
func NoiseDialer(addr string, identity cmtcrypto.PrivKey, validator peer.ID, timeout time.Duration) (privval.SocketDialer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kms: noise identity: %w", err)
	}
	tr, err := libp2pnoise.New(libp2pnoise.ID, lpk, nil)
	if err != nil {
		return nil, fmt.Errorf("kms: noise transport: %w", err)
	}
	return func() (net.Conn, error) {
		...
		if err != nil {
			_ = raw.Close()
			return nil, fmt.Errorf("kms: noise handshake with %s: %w", validator, err)
		}
		return secured, nil
	}, nil
}
