...
func (s *Signer) Sign(ctx context.Context, payload []byte) ([]byte, error) {
	...
	if s.algo.msgType == types.MessageTypeRaw && len(msg) > kmsRawMessageLimit {
		return nil, fmt.Errorf("awskms: %d-byte payload exceeds the %d-byte AWS KMS RAW message limit, which scheme %s cannot sign (e.g. large vote-extension sign-bytes)",
